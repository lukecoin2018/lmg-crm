...
interface UseDiscoveryReturn {
  ...
  discoverTikTok: (tiktokHandle: string) => Promise<void>;
  discoverYouTube: (youtubeHandle: string) => Promise<void>; // NEW: YouTube support
 ...
 * 
 * Usage:
 * const { discover, discoverTikTok, discoverYouTube, result, loading, error, progress } = useDiscovery(userId);
 * 
 ...
 * 
 * // Trigger YouTube discovery
 * await discoverYouTube('channelhandle');
 * 
 * // Trigger TikTok discovery
 * await discoverTikTok('username');
 * 
...
export function useDiscovery(userId: string): UseDiscoveryReturn {
  ...
  /**
   * Discover brand opportunities via YouTube
   */
  const discoverYouTube = async (youtubeHandle: string) => {
    try {
      setLoading(true);
      setError(null);
      setResult(null);
      ...
      setProgress({
        stage: 'discovery',
        message: 'Searching YouTube for similar creators...',
      });

      const response = await fetch('/api/scout/discover/youtube', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return {
    discover,
    discoverTikTok,
    discoverYouTube, // NEW: Export YouTube function
    result,
    loading,
    error,
    progress,
